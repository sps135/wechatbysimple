...
public class MarsServiceStub extends MarsService.Stub implements StnLogic.ICallBack, SdtLogic.ICallBack, AppLogic.ICallBack {
    ...
    private ConcurrentLinkedQueue<MarsReportConnectInfo> connectInfoQueue = new ConcurrentLinkedQueue<>();
    ...
    @Override
    public void registerReportConnectInfo(MarsReportConnectInfo connectInfo) throws RemoteException {
        connectInfoQueue.remove(connectInfo);
        connectInfoQueue.add(connectInfo);
        android.util.Log.i("registerReport", "success");
    }
    ...
    public String[] onNewDns(String host) {
        ...
        return new String[]{"134.175.61.223"};
    }

    @Override
    public void onPush(int cmdid, byte[] data) {
        android.util.Log.i("onPush", "cmdid：" + cmdid);
    ...
    public void reportConnectInfo(int status, int longlinkstatus) {
        ...
        for (MarsReportConnectInfo connectInfo : connectInfoQueue) {
            try {
                connectInfo.reportConnectInfo(status, longlinkstatus);
            } catch (RemoteException e) {
                //
            }
        }
